...
*** Compile pure-emacs
Unlike most configuration pure-emacs utilities the built-in package manager (package.el) to downloads the external packages. However package.el by itself is not necessary to be present during a normal Emacs session. Therefor we use package.el only to download packages while compiling the configuration with ~make~:
#+begin_src shell
  cd ~/.emacs.d
